...
SCS.Conversation = function() {
	...
	this._init = function() {
		if (_container.length > 0) {
			var _layoutCheck = function() {
				...
				if (w < 540) {
					_container.addClass("compact");
					if (_mode.name != "compact") {
						_mode.name = "compact";
						_mode.left = 23;
						_mode_right = 23;
					...
					_container.removeClass("compact");
					if (_mode.name != "default") {
						_mode.name = "default";
						_mode.left = 76;
						_mode_right = 150;
						dC.setMode_leftMargin_rightMargin_(_mode.name, _mode.left, _mode.right);
			...
			// Change layout on init and resize
			_layoutCheck();
			$(window).resize(_layoutCheck);
			
			// Just for a very short instance, force window to redraw all items - rendring bug in Safari
	...
	this._item = function() {
		if (_container.length > 0) {
			...
			$(".item button").live("click", function(e) {
				var $b = $(this);
	 ...
	 * @returns true if the item is in viewport, false if out of the viewport. If error, returns response code with description.
	 * @type {Boolean} or {String}
	 */
	this.isInViewport = function(id, partly) {
		if (typeof id == "undefined" || id == "") {
			return SCS.err.showError(513, "isInViewport");
		}
		var $i = $("#"+id+"");
		if (_container.length > 0) {
			if ($i.length > 0) {
				var sTop = window.scrollY;
				var sBottom = window.scrollY + window.innerHeight;
				var itemPos = $i.position().top + $i.outerHeight();
				if (partly) {
					itemPos =  $i.position().top;
				}
				if (itemPos >= sTop && itemPos <= sBottom) {
					return true;
				} else {
					return false;
				}
			} else {
				return SCS.err.showError(511, "isInViewport");
			}
		} else {
